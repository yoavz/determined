...
func findMultiSlotFits(
	...
	// 1) Multi-agent tasks will have `DedicatedAgent` enabled. This is currently
	//    a valid assumption because the only multi-machine tasks we have
	//    is distributed training, which requires dedicated instances.
	...
	for _, n := range numSlots {
		...
		if s := task.SlotsNeeded(); maxSlots >= s {
			if task.fittingRequirements.DedicatedAgent && s%n != 0 {
				continue
			}
	...
	if task.SlotsNeeded() > candidateNumSlots && !task.fittingRequirements.DedicatedAgent {
		log.Debug("Placement of multi-machine tasks without requiring dedicated " +
			"agents is not currently supported.",
		)
		return nil
	}
